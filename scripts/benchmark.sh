...
echo "------                              ---  -------  ------"
"$REF_GEN" -n 100 -o /tmp/yodel_bench_noise.wav >/dev/null 2>&1
printf "%-35s %4s %8s %7s\n" "synthetic-noise-100" \
    "$(count_ref /tmp/yodel_bench_noise.wav)" \
    "$(count_ref_ep /tmp/yodel_bench_noise.wav)" \
    "$(count_yodel /tmp/yodel_bench_noise.wav)"
...
"$REF_GEN" -B 300 -n 100 -o /tmp/yodel_bench_noise_300.wav >/dev/null 2>&1
printf "%-35s %4s %8s %7s\n" "synthetic-noise-100-300bd" \
    "$(count_ref_300 /tmp/yodel_bench_noise_300.wav)" \
    "$(count_ref_300_ep /tmp/yodel_bench_noise_300.wav)" \
    "$(count_yodel_300 /tmp/yodel_bench_noise_300.wav)"
# Additive 9600-baud G3RUH row (does not touch the five pinned rows above).
"$REF_GEN" -B 9600 -n 100 -o /tmp/yodel_bench_noise_9600.wav >/dev/null 2>&1
printf "%-35s %4s %8s %7s\n" "synthetic-noise-100-9600bd" \
    "$(count_ref_9600 /tmp/yodel_bench_noise_9600.wav)" \
    "$(count_ref_9600_ep /tmp/yodel_bench_noise_9600.wav)" \
    "$(count_yodel_9600 /tmp/yodel_bench_noise_9600.wav)"
...
"$REF_GEN" -X 1 -n 100 -o /tmp/yodel_bench_noise_fx25.wav >/dev/null 2>&1
printf "%-35s %4s %8s %7s\n" "synthetic-noise-100-fx25" \
    "$(count_ref /tmp/yodel_bench_noise_fx25.wav)" \
    "$(count_ref_ep /tmp/yodel_bench_noise_fx25.wav)" \
    "$(count_yodel_fx25 /tmp/yodel_bench_noise_fx25.wav)"
echo
